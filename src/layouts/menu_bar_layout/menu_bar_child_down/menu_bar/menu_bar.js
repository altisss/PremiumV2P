...
class Menubar extends React.Component {
  ...
  handler_menu_button(sub, index1, index2) {
    ...
    if (themes.includes(sub.routerLink)) {
      ...
      // if (sub.title === 'common_index_chart_info') {
      //   const menu = this.state.dataMenu
      //   menu[index1].subMenu[index2].disable = true
      //   this.setState({ dataMenu: menu })
      //   this.props.onAddLayout(sub.routerLink, sub.title, {index1, index2 })
      // }
      // else  {
  ...
  openPlaceOrder = (item) => {
    ...
    if (item.title === 'add_order') {
      const component = sub.routerLink + this.count_add_order
      this.props.onAddLayout(component, sub.title, { component, index1: 6, index2: 0 })
      this.count_add_order++
    } else return;
  }
