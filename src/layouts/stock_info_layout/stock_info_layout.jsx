...
import { get_glb_sv } from '../../utils/get_glb_or_socket_sv';
...
class StockInfoLayout extends React.PureComponent {
  ...
  //-- Lấy tin tức chuyên sâu
  getDeepNewsList = (stk) => {
    if (this.getDeepNewsListFlag) { return; }
    // const clientSeq = socket_sv.getRqSeq();
    ...
    reqInfo.component = this.component
    reqInfo.receiveFunct = this.handle_getDeepNewsList;
    // -- service info
    let svInputPrm = new serviceInputPrm();
    // svInputPrm = glb_sv.constructorInputPrm(svInputPrm);
    // svInputPrm.ClientSeq = clientSeq;
    ...
    svInputPrm.ClientSentTime = '0';
    svInputPrm.Operation = 'Q';
    svInputPrm.InVal = ['03', stk];
    svInputPrm.TotInVal = svInputPrm.InVal.length;
    // console.log('getDeepNewsList',JSON.stringify(svInputPrm));
    // socket_sv.send2Sv(socket_sv.key_ClientReq, JSON.stringify(svInputPrm));
    this.getDeepNewsList_ReqTimeOut = setTimeout(this.functSolveTimeOut, functionList.reqTimeout, request_seq_comp);
    reqInfo.inputParam = svInputPrm.InVal;
    // glb_sv.setReqInfoMapValue(clientSeq, reqInfo);
    this.deepNewsList = [];
  ...
  handle_getDeepNewsList = (reqInfoMap, message) => {
    ...
    if (Number(message['Result']) === 0) {
      ...
      // console.log(JSON.stringify(this.deepNewsList));
      if (Number(message['Packet']) <= 0) {
        reqInfoMap.procStat = 2;
        this.getDeepNewsListFlag = false;
        this.setState({ deepNewsList: this.deepNewsList });
        // console.log('deepNewsList',this.deepNewsList);
        // glb_sv.setReqInfoMapValue(cltSeqResult, reqInfoMap);
        // if (this.deepNewsList && this.deepNewsList.length > 0) {
        //   setTimeout(() => this.handleChangeLinkIframe(this.deepNewsList[0],'deepnews'),0);
        // }
      }
    }
  }
