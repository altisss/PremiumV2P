...
class RightForBuy extends React.Component {
  ...
  loadData() {
    ...
    window.ipcRenderer.once(`${commuChanel.reply_get_value_from_glb_sv}_${this.component}_${sq}`, (event, args) => {
      ...
      const activeCode = args.get('activeCode')
      const activeAcnt = args.get('activeAcnt')


      glb_sv.objShareGlb['acntNoInfo'].forEach((item) => {
        ...
        const js_acntObj = {
          'id': item['AcntNo'] + '.' + item['SubNo'],
          'name': item['AcntNo'] + '.' + item['SubNo'] + ' - ' + item['AcntNm']
        }
        this.acntItems.push(js_acntObj);
      });
      // const acntStr = this.acntItems[0]['id'];
            ...
            <div className='page1'>
              <div className="form-group row ">
                ...
                <div className='col-sm-7 no-padding-left'>
                  {
                    this.state.acntItems && <SelectBasic
                      inputtype={"text"}
                      name={"rgt_acntNo"}
                      value={this.state.rghtForBuy.acntNo}
