...
class socketService {
  constructor() {
    ...
    */
    ...
    // -- Khởi tạo dữ liệu indexWebWorker -----------------------------WorkerReqMsgEp
    const { remote } = window.require('electron')
    console.log('My path:', remote.app.getAppPath())
    const path = require('path')
    this.indexWebWorker = new Worker(`${path.join(remote.app.getAppPath(), 'build/assets/WorkerReqMsgI.js')}`);
    this.epmsgWebWorker = new Worker(`${path.join(remote.app.getAppPath(), 'build/assets/WorkerReqMsgEp.js')}`);
