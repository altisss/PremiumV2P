...
class globalService {
  constructor() {
    ...
    // -- Khởi tạo dữ liệu indexWebWorker -----------------------------  `${path.join(remote.app.getAppPath(), 'build/assets/WorkerStoreStkList.js')}`
    this.simsgWebWorker = new Worker(`${path.join(remote.app.getAppPath(), 'build/assets/WorkerReqMsgSi.js')}`);
    this.storStkListWebWorker = new Worker(`${path.join(remote.app.getAppPath(), 'build/assets/WorkerStoreStkList.js')}`);
    ...
    this.update2MrkInfoMsgMap = (msgTp, msgObj, clientSeq) => {
      ...
      switch (msgTp) {
        ...
        case 'TP':
          ...
          break;
        case 'EP':
          console.log('vao update EP: ', this.objShareGlb['stkOrd']['t55']);
    ...
    this.updI_Msg2MrkInfoMap = (clientSeq = 0, msgObj = {}) => {
      ...
      if (calTime != null && calTime !== undefined && calTime.length >= 8) {
        ...
        if (Number(msgObj['U12']) === 0 && (msgObj['seq'] === 0 && msgObj['subseq'] === 0)) {
          ...
          newIndexNode = [[9, 0, 0], Number(msgObj['t3']), 0, Number(msgObj['t3'])];
          // return;
        }
      }
    ...
    this.updEP_Msg2MrkInfoMap = (clientSeq = 0, msgObj = {}) => {
      ...
      if (autionTimePrcObj != null && autionTimePrcObj.length > 0) {
        ...
        if (clientSeq > 0) {
          ...
          autionTimePrcObjChart.splice(2, 0, newObj);
        } else {
