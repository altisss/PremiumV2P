...
var UDFCompatibleDatafeedBase = /** @class */ (function() {
    function UDFCompatibleDatafeedBase(glbServ, socket, datafeedURL, quotesProvider, requester, updateFrequency) {
        if (updateFrequency === void 0) {
            updateFrequency = 1.5 * 1000
        }
        var _this = this
        ...
        this._dataPulseProvider = new DataPulseProvider(this._glbServ, this._historyProvider, updateFrequency)
    ...
    // lắng nghe thay đổi Mã CK trên tradingview
    UDFCompatibleDatafeedBase.prototype.subscribeBars = function(
        ...
        if (symbolInfo.exchange === 'HOSE') {
            stkID = 'HSX_' + symbolInfo.name
        } else {
            stkID = 'HNX_' + symbolInfo.name
        }
        const stkObj = this._glbServ.getMsgObjectByMsgKey(stkID)
        message['type'] = 'misTypeTradvStock'
        message['data'] = stkObj
        this._glbServ.stkTradEvent.next(JSON.stringify(message))
...
function defaultConfiguration() {
    return {
        ...
        supports_timescale_marks: false, // -- Dữ liệu có support các dấu thời gian không
        supports_time: true,
        exchanges: [
            { value: '', name: 'All Exchanges', desc: '' },
            { value: 'HOSE', name: 'HOSE', desc: 'HOSE' },
            { value: 'HNX', name: 'HNX', desc: 'HNX' },
            { value: 'UPC', name: 'UPC', desc: 'UPC' },
        ],
        symbols_types: [
            { name: 'All typestypes', value: '' },
            { name: 'Stock', value: 'stock' },
            { name: 'Index', value: 'index' },
        ],
        // supported_resolutions: ['1', '5', '15', '30', '60', 'D', 'W', 'M', '3M'],
        supported_resolutions: ['D', 'W', 'M', '3M'],
    }
    // return {
