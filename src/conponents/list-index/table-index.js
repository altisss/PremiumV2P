...
class TableIndexList extends React.Component {
  constructor(props) {
    ...
    window.ipcRenderer.once(`${commuChannel.reply_get_value_from_glb_sv}_${this.props.component}_${sq}`, (event, agrs) => {
        ...
        let list = VN_INDEX.indexArr.slice(-100);
        console.log(list)
		    // update_value_for_glb_sv({component: this.props.component, key: ['VN_INDEX', 'indexArr'], value: list})
        list.shift();
        objVN_INDEX.list = list.map(item => {
          return item[1]
        });
        objVN_INDEX.list.unshift(VN_INDEX.ref);
        ...
        list = HNX_INDEX.indexArr.slice(-100);
        // update_value_for_glb_sv({component: this.props.component, key: ['HNX_INDEX', 'indexArr'], value: list})
        list.shift();
        objHNX_INDEX.list = list.map(item => {
          return item[1]
        });
        objHNX_INDEX.list.unshift(HNX_INDEX.ref);
        ...
        list = UPCOM_INDEX.indexArr.slice(-100);
        // update_value_for_glb_sv({component: this.props.component, key: ['UPCOM_INDEX', 'indexArr'], value: list})
        list.shift();
        objUPCOM_INDEX.list = list.map(item => {
          return item[1]
        });
        objUPCOM_INDEX.list.unshift(UPCOM_INDEX.ref);
