...
class TableRowPriceboard extends React.Component {
  ...
  componentWillUnmount() {
    ...
    // if (this.timeSendTrdView) clearTimeout(this.timeSendTrdView);
    // if (glb_sv.timeSendTrdView) clearTimeout(glb_sv.timeSendTrdView);
  }
  ...
  sortOrderFLV = (stk) => {
    ...
    this.setState({orderBol: !this.state.orderBol});
  }
      ...
      <tr className={'rowTable ' + (this.state.t55 === this.state.selected ? 'active' : '')} style={{ height: '26px' }}>
        <th className={`c0pb stkNmFvl cursor_ponter ${this.state.t31_color} `} id={this.state.itemName + "t55"}>
          ...
          {this.state.type === 'WL' && <div className="info" style={{ display: 'inline' }}>
            <button title={this.props.t('see_info_invest_recommendation')} style={{ float: 'right', padding: '2px 2px 0 0', color: 'white', margin: 0, fontSize: 10 }} onClick={() => this.showNews(this.props.row.Link)} className="btn btn-sm btn-link cursor_ponter">
          ...
          <div className="info" style={{ display: 'inline' }}>
            <button title={this.state.orderBol ? this.props.t('remove_to_first_row') : this.props.t('move_to_first_row')} style={{ float: 'right', padding: '2px 6px 0 0', color: 'white', margin: 0, fontSize: 10 }} onClick={() => this.sortOrderFLV(this.state.t55)} className="btn btn-sm btn-link cursor_ponter">
              <i style={{color: (this.state.orderBol ? '#9a6512' : 'inherit')}} className="fa fa-arrow-up"></i>
            </button>
          </div>
          {this.state.type === 'FVL' && <div className="removeStk">
            <button style={{ float: 'right', padding: '2px 8px 0 0', color: 'white', margin: 0 }} onClick={() => this.props.removeStkFromFvl(this.state.t55)} className="btn btn-sm btn-link cursor_ponter">
